 ...
 */
@Component
public class BuyAirCompanyB {

    @BranchT(confirm = "doConfirm", cancel = "doCancel", order = Integer.MIN_VALUE)
    public BuyResponse doTry(BuyRequest buyRequest) {

        HoopLogger.info(">>Air B Company 开始预定机票,param:{}", buyRequest);

        BuyResponse buyResponse = new BuyResponse();
        buyResponse.setStatus(BuyResponse.Proccess);
        HoopLogger.info(">>Air B Company 执行 rpc调用 param:{} rpc返回结果:res:{}", buyRequest, buyResponse);

        if (buyResponse.getStatus().equals(BuyResponse.Failed)) {
            throw new RollbackException("航空公司B出票失败");
        } else if (buyResponse.getStatus().equals(BuyResponse.Proccess) || buyResponse.getStatus().equals(BuyResponse.UNKNOW)) {
            throw new SuspendException("航空公司B出票处理中 挂起");
        }
    ...
    public void doConfirm(BuyRequest buyRequest) {
        HoopLogger.info(">>Air B Company 开始执行commit操作,param:{}", buyRequest);
    }

    public void doCancel(BuyRequest buyRequest) {
        HoopLogger.info(">>Air B Company 开始执行cancel操作,param:{}", buyRequest);
    }
